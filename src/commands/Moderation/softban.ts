...
bot.utils.createSlashCommand({
  name: "softban",
  description:
    "Bans a member while deleting messages and unbanning after that.",
  category: "Moderation",
  guildOnly: true,
  options: [
    {
  ...
  execute(bot, data) {
    ...
    if (!member) return;
    const days = data.data?.options?.[1].type == 4
      ? (data.data?.options?.[1].value as number)
      : 7;
    const reason = (data.data?.options?.find((e) =>
      e.type == 3
    )?.value as string) ||
      "No Reason.";
    if (
      ...
      return sendResponse(
        bot,
        data,
        "I can't softban this member since they have a higher role than you.",
      );
    }
    if (
    ...
    bot.helpers.unbanMember(data.guildId!, bot.utils.snowflakeToBigint(member));
    const embed = new AmethystEmbed()
      .setTitle("SoftBan Command")
      .setDescription(
        `Successfuly softbanned <@${
